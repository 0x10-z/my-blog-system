...
draft: false
authors: ['default']
summary: 'Reto CTF que consiste en escanear los puertos de una máquina y descubrir varios puertos abiertos. En el puerto 80 encontramos un formulario vulnerable a XSS, donde insertamos un script para robar cookies. Al recibir la petición, verificamos que el ataque funciona correctamente. Descubrimos que el campo de país del formulario es vulnerable a inyecciones SQL. Aprovechamos esta vulnerabilidad para obtener una shell y acceder a la configuración del sistema. Finalmente, encontramos las flags de usuario y root.'
