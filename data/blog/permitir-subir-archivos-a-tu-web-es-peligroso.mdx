...
draft: false
authors: ['default']
summary: 'En este post, aprenderemos sobre el riesgo de permitir a los usuarios subir archivos a través de formularios en plataformas web. Analizaremos las vulnerabilidades que pueden ser objetivo de ataques, como la ejecución de código remoto en el contexto del cliente o del servidor. Además, presentaremos una prueba de concepto que demuestra cómo un archivo PHP puede ser utilizado para obtener acceso y controlar el sistema. También se discuten medidas preventivas, como validar el tipo de archivo, limitar el tamaño del archivo y almacenarlos en un lugar externo al servidor.'
