...
Podemos pensar que la segunda web es fraudulenta, por supuesto, pero estas actas físicas **tienen un código QR generado por el propio CNE** (más se muestra uno de ejemplo). Es decir, el CNE podría simplemente mostrar que la papeleta auténtica (la física) no se corresponde con lo publicado. **Esto aún no ha pasado.**
...
Es decir, **en este punto la CNE es quien tiene la sartén por el mango y es quien tiene el poder de dar veracidad a las cifras oficiales o desmentir a la oposición. Mientras no se sepa nada más, todo lo que se publique será humo y no podrá tomarse por cierto.**
