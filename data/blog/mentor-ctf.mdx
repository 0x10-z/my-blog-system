...
draft: true
authors: ['default']
summary: 'Reto CTF que consiste en realizar una fase de enumeración utilizando NMAP para escanear puertos abiertos en un servidor. Se analizan los puertos 22 y 80, encontrando información relevante como el servidor Apache y la existencia de un subdominio. Luego se realiza una enumeración web utilizando wfuzz para encontrar posibles subdominios y endpoints ocultos. También se descubre que el servidor tiene SNMP funcionando en el puerto 161 y se utiliza una herramienta de fuerza bruta para enumerar versiones y cadenas de comunidad disponibles. Se encuentra un subdominio vulnerable mediante SNMP y se obtiene una contraseña que se utiliza para realizar una solicitud a la API, obteniendo un token de acceso. A través de un endpoint vulnerable a RCE, se logra obtener una reverse shell y escalada de privilegios, accediendo así a la cuenta de root. Se completa el reto obteniendo las flags correspondientes. Vulnerabilidades: puertos abiertos, servidores vulnerables a RCE, SNMP sin seguridad.'
images: ["/static/images/uploads/mentor-ctf/Mentor.webp", "/static/images/uploads/mentor-ctf/image-6.webp", "/static/images/uploads/mentor-ctf/Untitled-2022-12-30-2011.svg", "/static/images/uploads/mentor-ctf/image-7.webp", "/static/images/uploads/mentor-ctf/image-8.webp", "/static/images/uploads/mentor-ctf/image-9.webp", "/static/images/uploads/mentor-ctf/image-10.webp", "/static/images/uploads/mentor-ctf/Untitled-2022-12-30-2011-1.svg"]
---

![](/static/images/uploads/mentor-ctf/Mentor.webp)
...
![](/static/images/uploads/mentor-ctf/image-6.webp)

mentorquotes.htbEn esta página no hay mucho que ver. Con *whatweb* extraemos la siguiente información:
...
![](/static/images/uploads/mentor-ctf/Untitled-2022-12-30-2011.svg)

Server schemaRepetimos el fuzzing contra el subdominio que acabamos de encontrar, a ver si hay alguna ruta oculta.
...
![](/static/images/uploads/mentor-ctf/image-7.webp)
...
![](/static/images/uploads/mentor-ctf/image-8.webp)

Captura de Burpsuite/RepeaterVemos que existen dos nuevos *endpoints*. */check* y */backup*. Parece que el primero no funciona, y el segundo, solo funciona si la petición es de tipo POST. Además, el parámetro *Content-Type* debe ser *application/json* y espera recibir un diccionario del siguiente tipo:
```
{"body": "xxx", "path": "xxx"}
```

![](/static/images/uploads/mentor-ctf/image-9.webp)

Captura de Burpsuite/RepeaterSegún parece, el valor que dejas en el parámetro path es vulnerable a *RCE*. Escuchamos en el puerto 1234 con *Netcat* y entablamos una reverse *shell*. Por un lado, tenemos lo siguiente:

![](/static/images/uploads/mentor-ctf/image-10.webp)

Captura de Burpsuite con la Reverse ShellY por el otro, entablamos la *shell*:
...
![](/static/images/uploads/mentor-ctf/Untitled-2022-12-30-2011-1.svg)

Pivoting exampleCon ayuda de [*Chisel*](https://github.com/jpillora/chisel), ejecutamos en nuestra máquina el siguiente comando:
