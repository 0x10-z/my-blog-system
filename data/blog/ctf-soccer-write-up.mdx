...
draft: true
authors: ['default']
summary: 'Reto CTF que consiste en encontrar vulnerabilidades y explotarlas. Se realiza una exploración con nmap y wfuzz, encontrando una ruta /tiny. Se accede al repositorio y se encuentra una vulnerabilidad RCE en Tiny File Manager 2.4.6. Se obtiene acceso al servidor y se encuentra un subdominio donde se pueden verificar tickets de futbol. Se utiliza un script y sqlmap como proxy para realizar sqli. Se encuentra un archivo con el bit SUID activo llamado doas, se crea un plugin para dstat y se ejecuta como root. Finalmente, se obtiene acceso de root al servidor.'
